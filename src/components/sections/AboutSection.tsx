  ...
  return (
    ...
    >
      ...
      <div className="block md:hidden space-y-4">
        ...
        {/* Development Quality - Compact */}
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20 p-4 rounded-lg border-2 border-blue-200 dark:border-blue-800">
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="space-y-2">
            {freelanceAbout.businessFacts.slice(0, 2).map((fact, index) => (
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="space-y-2">
            {freelanceAbout.businessSkills[0]?.skills.slice(0, 2).map((skill, index) => (
          ...
          <AnimatePresence>
            {showAllSkills && (
              ...
              >
                {freelanceAbout.businessSkills[0]?.skills.slice(2).map((skill, index) => (
                  <div key={index + 2} className="flex items-start space-x-2">
                    <CheckCircle className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <div className="font-medium text-sm">{skill.name[language]}</div>
                      <div className="text-xs text-muted-foreground">{skill.description[language]}</div>
          ...
          <button
            ...
            className="flex items-center space-x-1 text-sm text-primary font-medium hover:text-primary/80 transition-colors mt-3"
          >
            <span>
              {showAllSkills 
                ? (language === 'en' ? 'Show Less' : 'Toon Minder')
                : (language === 'en' ? 'Show All Services' : 'Toon Alle Diensten')
              }
            </span>
        ...
        <div className="bg-gradient-to-br from-primary/5 to-primary/10 p-4 rounded-lg border border-primary/20">
          ...
          {/* Always show first 2 reasons */}
          <div className="space-y-2">
            {freelanceAbout.whyChooseMe.slice(0, 2).map((reason, index) => (
              <div key={index} className="flex items-start space-x-2">
          ...
          <AnimatePresence>
            {showAllReasons && (
              <motion.div
                ...
                className="mt-3 space-y-2"
              >
                {freelanceAbout.whyChooseMe.slice(2).map((reason, index) => (
                  <div key={index + 2} className="flex items-start space-x-2">
                    <Star className="h-4 w-4 text-primary mt-0.5 flex-shrink-0" />
                    <div>
                      <div className="font-medium text-primary text-sm">{reason.title[language]}</div>
          ...
          {/* Show More Button */}
          {freelanceAbout.whyChooseMe.length > 2 && (
            <button
            ...
            >
              <span>
                {showAllReasons 
                  ? (language === 'en' ? 'Show Less' : 'Toon Minder')
                  : (language === 'en' ? 'Show More' : 'Toon Meer')
                }
              </span>
              {showAllReasons ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
            </button>
          )}
        </div>
    ...
    >
      ...
      <div className="block md:hidden space-y-4">
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="space-y-2">
            {professionalAbout.professionalFacts.slice(0, 3).map((fact, index) => (
              <div key={index} className="flex items-center space-x-2 text-muted-foreground text-sm">
                {iconMap[fact.icon]}
                <span className="font-medium">{fact.label[language]}:</span>
                <span>{fact.value[language]}</span>
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          <h3 className="text-lg font-semibold mb-3">
            {language === 'en' ? 'Core Skills' : 'Kernvaardigheden'}
          </h3>
          
          {/* Show top 3 skills from first category */}
          <div className="space-y-3">
            {professionalAbout.technicalSkills[0]?.skills.slice(0, 3).map((skill, index) => (
              <div key={index} className="space-y-1">
                ...
                <div className="w-full bg-muted rounded-full h-1.5">
                  <div 
                    className={`h-1.5 rounded-full transition-all duration-1000 ease-out ${getSkillColor(skill.level)}`}
                  ...
                  <div key={categoryIndex}>
                    <h4 className="font-semibold text-sm mb-2 flex items-center space-x-2">
                      <div className={`p-1 rounded ${
                        ...
                        {categoryIndex === 0 ? <Database className="h-3 w-3" /> :
                         categoryIndex === 1 ? <Globe className="h-3 w-3" /> :
                         <Cloud className="h-3 w-3" />}
                      </div>
                      <span>{getText(category.category)}</span>
                    </h4>
                    <div className="space-y-2">
                      {category.skills.slice(categoryIndex === 0 ? 3 : 0, categoryIndex === 0 ? undefined : 3).map((skill, skillIndex) => (
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="space-y-2">
            {professionalAbout.careerHighlights.slice(0, 2).map((highlight, index) => (
              <div key={index} className="flex items-start space-x-2">
            ...
            <div className="text-xs text-muted-foreground">
              +{professionalAbout.careerHighlights.length - 2} {language === 'en' ? 'more achievements' : 'meer prestaties'}
            </div>
          </div>
        </div>
      </div>
...
export default function AboutSection({ language, viewMode }: SectionProps) {
  ...
  return (
    <section id="about" className="section-padding py-8 md:py-16 lg:py-20">
